import {
  ...
  export const loader = async ({ request }) => {
    ...
    `;
      ...
      const locallyProducedValue = metafields.find((m) => m.key === "locally_produced")?.value === "true";
  ...
  export default function LocallyProducedPage() {
    ...
    return (
      <Page>
        ...
        <Layout>
          <Layout.Section>
            <Text variant="headingLg" as="h1">Locally produced ratio: {((locallyProduced.length / (locallyProduced.length + internationallyProduced.length)) * 100).toFixed(0)}%</Text>
            <Text>
              We found {locallyProduced.length} locally produced products and {internationallyProduced.length} internationally produced products.
            </Text>
          </Layout.Section>
          <Layout.Section>
            <Text>Toggle the checkbox if your product's production place has changed.</Text>
          </Layout.Section>
          ...
          <Layout.Section oneHalf>
            {renderProductList(internationallyProduced, "attention", "Internationally Produced")}
          </Layout.Section>
        </Layout>
      </Page>
    );
