    ...
    try {
      ...
      for (const definition of definitionsToCreate) {
        try {
          ...
          console.error(`Failed to create metafield definition ${definition.key}:`, err);
        ...
        for (const productEdge of products) {
          ...
          if (metafieldsToSet.length > 0) {
            try {
              ...
              console.error(`Error setting default metafields for product ${shopifyProduct.title}:`, err);
            }
          }
        }
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    const metafieldsReady = !needsMetafieldSetup || 
  ...
  }, [needsImport, needsMetafieldSetup, isImporting, importFetcher, metafieldFetcher.data]);
              ...
              <BlockStack gap="500">
                <BlockStack gap="200">
                  ...
                  <Text variant="bodyMd" as="p">
                    Track sustainability metrics for your products and improve your store's environmental impact.
                  </Text>
                </BlockStack>
                ...
                {isCreatingMetafields && (
                  <Banner title="Setting Up Sustainability Metrics" tone="info">
                    <p>We're setting up the necessary fields to track sustainability metrics for your products...</p>
                  </Banner>
                )}
                
                {metafieldFetcher.data?.success && (
                  <Banner title="Metrics Setup Complete" tone="success">
                    <p>Successfully set up sustainability metrics tracking for your store.</p>
                  </Banner>
                )}
                
                {metafieldFetcher.data?.success === false && (
                  <Banner title="Setup Failed" tone="critical">
                    <p>Failed to set up sustainability metrics: {metafieldFetcher.data.error}</p>
                    <Button onClick={() => metafieldFetcher.submit({ action: "create_metafield_definitions" }, { method: "POST" })}>
                      Try Again
                    </Button>
                  </Banner>
                )}
                ...
                {!isImporting && !importFetcher.data && store.productCount > 0 && (
                  <Banner title="Products Ready" tone="success">
                    <p>You have {store.productCount} products ready for sustainability tracking.</p>
                  </Banner>
                )}
              ...
              <Card>
                <BlockStack gap="200">
                  ...
                  <List>
                    ...
                    <List.Item>
                      Monitor shipping distances
                    </List.Item>
                    <List.Item>
                      Manage packaging efficiency
                    </List.Item>
                    <List.Item>
